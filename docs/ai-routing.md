...
ただし、ChatGPT(Codex)とClaude Code間の伝達経路は、`docs/adr/0002-chatgpt-claude-code-local-bridge.md` の決定によりMCP経由で接続している。これは意図分類やAgent選択ロジックの自動化ではなく、人間が担っていたコピー&ペーストのリレーをツール呼び出しに置き換えるものであり、本節の「自動ルーティングエンジン」には該当しない。
