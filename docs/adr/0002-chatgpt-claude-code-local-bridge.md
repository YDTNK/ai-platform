...
- 生のMCP stdioプロトコルで `initialize` → `tools/call` を直接実行し、`sandbox: read-only` かつ `approval-policy: never` のセッションが正しい権限プロファイル（`file_system: restricted, access: read`）で起動することを確認した（レスポンスの内容自体はChatGPT/Codexアカウントの利用上限のため未取得。設定・配線面は検証済み）
