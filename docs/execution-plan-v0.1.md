...
本書はdocs/requirements-v0.1.mdとRoadmapで定義されたPhaseを、担当・入力・出力・依存関係・検証方法・完了条件を持つ実行可能なTaskへ分解する。コマンド、作成ファイル、実装順、Test、Completion Gateはdocs/implementation-guide-v0.1.mdを実行正本とする。
...
Phase 3〜7はdocs/implementation-guide-v0.1.mdの各Phase Sectionを、そのPhaseの親Taskとして使用する。

- Phase 3: Python Project、Domain、Persistence、API、CLI、Migration、CI
- Phase 4: Provider Adapter、Routing、MCP、GitHub、Retry、Trace
- Phase 5: Voice Session、Transcript、Summary、Decision、Task
- Phase 6: Loader、Index、Permission、Search、Citation、Evaluation
- Phase 7: PostgreSQL、Auth、Tenant、Deployment、Backup、Security、Privacy

各SectionのCommandをIssueへ複製せず、Issueから本書とImplementation GuideへLinkする。実行結果だけをIssueとPRへ記録する。
