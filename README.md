...
YDTNK/engineering-career-hq/projects/ai-platform
  └─ AI PlatformのProject管理・進捗・daily-log・fragment・review-taskの正本

YDTNK/ai-platform
  └─ AI Platform本体の設計・コード・テスト・Issue・PR・Commitの正本
```

AI Platformは `00-ai-start-here` を置き換えません。`00-ai-start-here` が「どこを見るか」を決め、AI Platformが「どう分解し、どのAI・ツールで実行するか」を扱います。

Projectの現在地、日次記録、判断履歴、次回タスクは `YDTNK/engineering-career-hq/projects/ai-platform/` で管理します。このRepositoryでは並行するdaily-report構造を持ちません。
...
GitHub: Branch・Commit・Pull Request・履歴
  ↓
engineering-career-hq/projects/ai-platform:
progress・daily-log・fragment・review-task
  ↓
